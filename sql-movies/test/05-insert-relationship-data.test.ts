...
const insertMovieGenres = (
  ...
  filtred.map(genre => `(${movieId}, ${genre.id})`).join(",");
};
...
const insertMovieActors = (
  ...
  var filtred = actorRows.filter(actorRows => actors.includes(actorRows.full_name));
  return `insert into ${MOVIE_ACTORS} (movie_id, actor_id) values`+
  filtred.map(actor => `(${movieId}, ${actor.id})`).join(",");
};
...
const insertMovieDirectors = (
  ...
  var filtred = directorRows.filter(directorRows => directors.includes(directorRows.full_name));
  return `insert into ${MOVIE_DIRECTORS} (movie_id, director_id) values`+
  filtred.map(director => `(${movieId}, ${director.id})`).join(",");
};
...
const insertMovieKeywords = (
  ...
  var filtred = keywordRows.filter(keywordRows => keywords.includes(keywordRows.keyword));
  return `insert into ${MOVIE_KEYWORDS} (movie_id, keyword_id) values`+
  filtred.map(keyword => `(${movieId}, ${keyword.id})`).join(","); 
};
...
const insertMovieProductionCompanies = (
  ...
  var filtred = productionCompanyRows.filter(productionCompanyRows => productionCompanies.includes(productionCompanyRows.company_name));
  return `insert into ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) values` +
  filtred.map(productionCompany => `(${movieId}, ${productionCompany.id})`).join(",");
};
